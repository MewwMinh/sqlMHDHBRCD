...
WITH daily AS (
    ...
    AND HEN_SUA_TU IS NOT NULL
    AND NGAY_HT IS NOT NULL
    AND TRANG_THAI_BH = 6
    AND CHUYEN_PHIEU NOT IN (1,2,4)
    AND (
        (NGAY_HT BETWEEN HEN_SUA_TU AND HEN_SUA_DEN)
        OR (NGAY_HT < HEN_SUA_TU AND (HEN_SUA_TU - NGAY_HT) * 24 <= 2)
        OR (NGAY_HT > HEN_SUA_DEN AND (NGAY_HT - HEN_SUA_DEN) * 24 <= 2)
        OR (HEN_SUA_DEN IS NULL AND ABS((NGAY_HT - HEN_SUA_TU) * 24) <= 2)
        OR (HEN_SUA_TU IS NULL AND ABS((NGAY_HT - HEN_SUA_DEN) * 24) <= 2)
    )
...
SELECT
    ...
    KPI_VALUE,
    MA_TINH,
    'Số phiếu đạt thời gian KPSC (có hẹn KH)' AS KPI_NAME,
